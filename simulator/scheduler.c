...
void handle_kvs_queue (ctx_t *ctx)
{
	...
	while (zlist_size (ctx->kvs_queue) > 0){
		...
		flux_log (ctx->h, LOG_DEBUG, "Event to be handled - key: %s, val: %s", kvs_event->key, kvs_event->val);
...
void newlwj_cb (const char *key, int64_t val, void *arg, int errnum)
{
    ...
    if (reg_lwj_state_hdlr (h, path, (KVSSetStringF *) queue_kvs_cb) == -1) {
        flux_log (h, LOG_ERR,
...
int extract_lwjinfo (ctx_t *ctx, flux_lwj_t *j)
{
    ...
    if (asprintf (&key, "lwj.%ld.state", j->lwj_id) < 0) {
        ...
        flux_log (ctx->h, LOG_ERR, "extract_lwjinfo %s: %s", key, strerror (errno));
        goto ret;
    } else {
    ...
    if (asprintf (&key, "lwj.%ld.nnodes", j->lwj_id) < 0) {
        ...
        flux_log (ctx->h, LOG_ERR, "extract_lwjinfo get %s: %s",
                  key, strerror (errno));
        goto ret;
    } else {
        j->req.nnodes = reqnodes;
        flux_log (ctx->h, LOG_DEBUG, "extract_lwjinfo got %s: %ld", key, reqnodes);
        free(key);
    }
    ...
    if (asprintf (&key, "lwj.%ld.ntasks", j->lwj_id) < 0) {
        ...
        flux_log (ctx->h, LOG_DEBUG, "extract_lwjinfo got %s: %ld", key, reqtasks);
        free(key);
        j->alloc.nnodes = 0;
        j->alloc.ncores = 0;
        j->rdl = NULL;
    ...
    if (asprintf (&key, "lwj.%ld.io_rate", j->lwj_id) < 0) {
        ...
        flux_log (ctx->h, LOG_DEBUG, "extract_lwjinfo got %s: %ld", key, io_rate);
        free(key);
    }
...
int action_j_event (ctx_t *ctx, flux_event_t *e)
{
     ...
     */
    /*
	flux_log (ctx->h, LOG_DEBUG, "attempting job %ld state change from %s to %s",
...
//        move_to_c_queue (e->lwj);
    ...
    default:
        flux_log (ctx->h, LOG_ERR, "job %ld unknown state %d",
...
bad_transition:
    flux_log (ctx->h, LOG_ERR, "job %ld bad state transition from %s to %s",
              e->lwj->lwj_id, stab_rlookup (jobstate_tab, e->lwj->state),
                              stab_rlookup (jobstate_tab, e->ev.je));
    return -1;
}
...
int update_job (ctx_t *ctx, flux_lwj_t *job)
{
    ...
    if (update_job_state (ctx, job, j_allocated)) {
        flux_log (ctx->h, LOG_ERR, "update_job failed to update job %ld to %s",
                  job->lwj_id, stab_rlookup (jobstate_tab, j_allocated));
    } else if (update_job_resources(ctx, job)) {
		kvs_commit (ctx->h);
        flux_log (ctx->h, LOG_ERR, "update_job %ld resrc update failed", job->lwj_id);
...
int update_job_state (ctx_t *ctx, flux_lwj_t *job, lwj_event_e e)
{
    ...
    if (!strcmp (state, "unknown")) {
        ...
        flux_log (ctx->h, LOG_ERR, "update_job_state %ld state update failed: %s",
        ...
        flux_log (ctx->h, LOG_ERR, "update_job_state %ld %s-time failed: %s",
                  job->lwj_id, state, strerror (errno));
    } else {
        rc = 0;
        flux_log (ctx->h, LOG_DEBUG, "updated job %ld's state in the kvs to %s",
                  job->lwj_id, state);
    }
...
int update_job_resources (ctx_t *ctx, flux_lwj_t *job)
{
    ...
    if (jr)
        ...
        flux_log (ctx->h, LOG_ERR, "update_job_resources passed a null resource");
                      ...
                      uint64_t *pnode, uint32_t *pcores)
{
    ...
    if (jr) {
        ...
        if (o) {
            //flux_log (ctx->h, LOG_DEBUG, "updating: %s", json_object_to_json_string (o));
        } else {
            flux_log (ctx->h, LOG_ERR, "update_job_cores invalid resource");
            rc = -1;
            goto ret;
        }
    } else {
        flux_log (ctx->h, LOG_ERR, "update_job_cores passed a null resource");
        rc = -1;
        goto ret;
    }
    ...
    if (imanode) {
        if (asprintf (&key, "lwj.%ld.rank.%ld.cores", job->lwj_id,
                      *pnode) < 0) {
            flux_log (ctx->h, LOG_ERR, "update_job_cores key create failed");
            rc = -1;
            goto ret;
        } else if (kvs_put_int64 (ctx->h, key, *pcores) < 0) {
            flux_log (ctx->h, LOG_ERR, "update_job_cores %ld node failed: %s",
                      job->lwj_id, strerror (errno));
            rc = -1;
            goto ret;
        }
...
int release_resources (ctx_t *ctx, struct rdl *rdl, const char *uri, flux_lwj_t *job)
{
    ...
    if (jr) {
        ...
        flux_log (ctx->h, LOG_ERR, "release_resources failed to get resources: %s",
                  strerror (errno));
    }
    free (lwjtag);
...
int release_lwj_resource (ctx_t *ctx, struct rdl *rdl, struct resource *jr, char *lwjtag)
{
    ...
    if (curr) {
        ...
        flux_log (ctx->h, LOG_ERR, "release_lwj_resource failed to get %s", uri);
        rc = -1;
    }
    free (uri);
...
static void deallocate_bandwidth_helper (ctx_t *ctx, struct rdl *rdl, struct resource *jr,
    ...
    if (curr) {
        ...
        if (strcmp (type, CORETYPE) == 0) {
          ...
          while ((child = rdl_resource_next_child (jr))) {
              deallocate_bandwidth_helper (ctx, rdl, child, io_rate, ancestors);
              rdl_resource_destroy (child);
          }
          zlist_pop (ancestors);
        }
        ...
        flux_log (ctx->h, LOG_ERR, "deallocate_bandwidth_helper failed to get %s", uri);
    }
    free (uri);
...
{
	flux_log (ctx->h, LOG_DEBUG, "deallocate_bandwidth job: %ld, io_rate: %ld", job->lwj_id, job->req.io_rate);
    ...
    if (jr) {
        ...
        flux_log (ctx->h, LOG_ERR, "deallocate_bandwidth failed to get resources: %s",
                  strerror (errno));
    }
...
void deallocate_resource_bandwidth (ctx_t *ctx, struct resource *r, int64_t amount)
{
    ...
    if (new_alloc_bw < 0) {
      flux_log (ctx->h, LOG_ERR, "too much bandwidth deallocated (%ld) - %s",
                amount, Jtostr (o));
    }
...
bool allocate_core (flux_lwj_t *job, struct resource *resource,
...
int init_and_start_scheduler (flux_t h, ctx_t *ctx, zhash_t *args, struct flux_msghandler *tab)
{
    ...
    if (!(rdllib = rdllib_open ()) || !(ctx->rdl = rdl_loadfile (rdllib, path))) {
    ...
    if ((r = rdl_resource_get (ctx->rdl, ctx->uri))) {
        ...
        if (idlize_resources (r)) {
            flux_log (h, LOG_ERR, "failed to idlize %s: %s", ctx->uri,
                      strerror (errno));
            rc = -1;
            goto ret;
        }
        ...
        flux_log (h, LOG_ERR, "failed to get %s: %s", ctx->uri,
                  strerror (errno));
        rc = -1;
        goto ret;
    }
    ...
    if (reg_newlwj_hdlr (h, (KVSSetInt64F*) newlwj_cb) == -1) {
        flux_log (h, LOG_ERR,
                  ...
                  const char *uri, int64_t free_cores, flux_lwj_t *job)
{
    ...
    if (free_cores > 0 && free_rdl) {
        ...
        flux_log (ctx->h, LOG_DEBUG, "schedule_job called on job %ld, no cores available, skipping", job->lwj_id);
        goto ret;
    }

	flux_log (ctx->h, LOG_DEBUG, "schedule_job called on job %ld, request cores: %d, free cores: %ld", job->lwj_id, job->req.ncores, free_cores);
	...
	if (free_root && free_cores >= job->req.ncores) {
        ...
        if (allocate_resources (rdl, uri, free_root, a, job, ancestors)) {
            ...
            flux_log (ctx->h, LOG_DEBUG, "not enough resources to allocate, rolling back");
            ...
            if (rdl_accumulator_is_empty(a)) {
                flux_log (ctx->h, LOG_DEBUG, "no resources found in accumulator");
            } else {
                job->rdl = rdl_accumulator_copy (a);
        ...
        flux_log (ctx->h, LOG_DEBUG, "not enough available cores, skipping this job");
    }
