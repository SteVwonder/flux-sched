...
JSON serialized_resrc_tree = NULL;
...
static int load_resources (ssrvctx_t *ctx)
{
    ...
    serialized_resrc_tree = Jnew ();
    if (resrc_tree_serialize (serialized_resrc_tree, resrc_phys_tree (ctx->rctx.root_resrc)) < 0) {
        flux_log (ctx->h, LOG_ERR, "%s: failed to serialized resrc_tree", __FUNCTION__);
    }
 ...
 */
int schedule_job (ssrvctx_t *ctx, sched_ctx_t *plugin_ctx, resrc_t *root_resrc, flux_lwj_t *job, int64_t starttime)
{
    JSON req_res = NULL;
    flux_t h = ctx->h;
    int rc = -1, child_rc = -1;
    ...
    if ((nfound = plugin->find_resources (h, root_resrc,
        ...
        if ((selected_tree = plugin->select_resources (h, plugin_ctx, found_tree,
            ...
            if (resrc_reqst_all_found (resrc_reqst)) {
                ...
                if (root_resrc == ctx->rctx.root_resrc) {
...
static int reserve_job (ssrvctx_t *ctx, sched_ctx_t *plugin_ctx, resrc_t *root_resrc, flux_lwj_t *job, int64_t starttime, int64_t walltime)
{
    JSON req_res = NULL;
    flux_t h = ctx->h;
    int rc = -1;
...
static int get_job_reservation_starttime (resrc_tree_t *resrc_tree, int64_t jobid, int64_t *starttime)
{
    ...
    if (resrc_reservation_starttime (resrc_tree_resrc (resrc_tree), jobid, starttime) >= 0) {
        rc = 0;
        goto done;
    }
    ...
    for (child = resrc_tree_list_first (children);
         child;
         child = resrc_tree_list_next (children)) {
        if (get_job_reservation_starttime (child, jobid, starttime) >= 0) {
            rc = 0;
            goto done;
        }
    }
...
static void print_job_reservation_starttimes (ssrvctx_t *ctx, const char* predictor)
{
    ...
    outfile = zhash_lookup (outfiles, predictor);
    if (!outfile) {
        if (ctx->arg.results_dir) {
            dir = ctx->arg.results_dir;
        } else {
            dir = ".";
        }

        asprintf (&filename, "%s/%s_predictions.out", dir, predictor);
        outfile = fopen (filename, "w");
        zhash_insert (outfiles, predictor, outfile);
        flux_log (ctx->h, LOG_DEBUG, "%s: Saving %s predictions to %s", __FUNCTION__, predictor, filename);
        free (filename);
    }
...
static int64_t get_actual_job_runtime (flux_t h, int64_t lwj_id)
{
    ...
    if (kvs_get_double (h, key, &execution_time) < 0) {
        flux_log_error (h, "extract %s", key);
    }
    else {
        flux_log (h, LOG_DEBUG, "extract %s: %f", key, execution_time);
    }
...
static int schedule_jobs (ssrvctx_t *ctx)
{
    ...
    int64_t starttime = (ctx->sctx.in_sim) ?
        (int64_t) ctx->sctx.sim_state->sim_time : epochtime();
    resrc_t *root_resrc = ctx->rctx.root_resrc;

    resrc_tree_release_all_reservations (resrc_phys_tree (root_resrc));
    if (!plugin)
        return -1;
    rc = plugin->sched_loop_setup (ctx->h, plugin_ctx);
