...
static void sim_res_event_cb (flux_t h, flux_msg_watcher_t *w,
                              const flux_msg_t *msg, void *arg) {
    ssrvctx_t *ctx = getctx ((flux_t)arg);
...
static struct flux_msghandler sim_htab[] = {
...
static int reg_sim_events (ssrvctx_t *ctx)
{
    ...
    if (flux_msg_watcher_addvec (ctx->h, sim_htab, (void *)h) < 0) {
        flux_log (ctx->h, LOG_ERR, "flux_msg_watcher_addvec: %s", strerror (errno));
        goto done;
    }
